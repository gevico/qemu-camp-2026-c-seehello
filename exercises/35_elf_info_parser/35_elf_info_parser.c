...
static void fix_ehdr_endian(const Elf64_Ehdr *src, Elf64_Ehdr *dst, int file_is_le, int host_is_le) {
    if (file_is_le == host_is_le) {
        ...
        /* 端序不同，逐字段转换 */
        dst->e_ident[EI_MAG0] = src->e_ident[EI_MAG0];
        dst->e_ident[EI_MAG1] = src->e_ident[EI_MAG1];
        dst->e_ident[EI_MAG2] = src->e_ident[EI_MAG2];
        dst->e_ident[EI_MAG3] = src->e_ident[EI_MAG3];
        dst->e_ident[EI_CLASS] = src->e_ident[EI_CLASS];
        dst->e_ident[EI_DATA] = src->e_ident[EI_DATA];
        dst->e_ident[EI_VERSION] = src->e_ident[EI_VERSION];
        dst->e_ident[EI_OSABI] = src->e_ident[EI_OSABI];
        dst->e_ident[EI_ABIVERSION] = src->e_ident[EI_ABIVERSION];
        memset(dst->e_ident + EI_PAD, 0, EI_NIDENT - EI_PAD); /* 填充剩余部分 */
