/**
 * test_02_merge_sort.c
 * 
 * 测试第二个练习题：归并排序
 * 输入：从文件读入 n 个学生的信息（包含姓名和成绩），n 不超过 100
 * 输出：使用归并排序算法按成绩从高到低排序后，在屏幕上依次显示排序后的学生姓名和成绩，每个学生信息占一行
 */

 #include "../checker/test_framework.h"

 int main() {
     test_init("02_merge_sort");
     
     printf("\n🧪 测试练习题: 02_merge_sort\n");
     
     // 检查文件是否还包含 "I AM NOT DONE" 标记
     int has_marker = file_contains_marker("../exercises/02_merge_sort.c", "I AM NOT DONE");
     ASSERT_FALSE(has_marker, "练习题应该移除 'I AM NOT DONE' 标记");
     
     if (has_marker) {
         printf("\n💡 提示: 请编辑 exercises/02_merge_sort.c 文件，移除 'I AM NOT DONE' 标记并完成代码\n");
         printf("💡 提示: 归并排序算法：分治法，将数组分成两半，分别排序后合并\n");
     ...
     // 编译并运行程序
     char output[1024];
     int result = compile_and_run("02_merge_sort", "02_merge_sort", output, sizeof(output), 0);
     ...
     // 检查是否包含开始提示
     ASSERT_TRUE(string_contains(output, "merge sort") || string_contains(output, "归并排序"), 
                 "输出应该包含排序算法相关信息");
     ...
     // 校验排序后的精确输出顺序与分数（基于 02_students.txt）
     ...
     if (line_count >= 2) {
         printf("📝 程序正确执行了归并排序算法\n");
         printf("💡 知识点: 归并排序时间复杂度O(n log n)，空间复杂度O(n)，稳定排序\n");
