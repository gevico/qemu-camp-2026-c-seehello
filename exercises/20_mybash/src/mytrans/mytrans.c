...
int load_dictionary(const char *filename, HashTable *table,
  ...
  while (fgets(line, sizeof(line), file)) {
    ...
    if (in_entry && current_translation[0] != '\0') {
      size_t used = strlen(current_translation);
      size_t avail = sizeof(current_translation) - used - 1;
      if (avail > 1) {
        strncat(current_translation, "@", avail);
        used = strlen(current_translation);
        avail = sizeof(current_translation) - used - 1;
        strncat(current_translation, line, avail);
      }
    }
  }
